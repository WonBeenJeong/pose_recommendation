...
def detector(filename):
    ...
    print("has read in file...\n")
    im = imutils.resize(im, width = min(400, im.shape[1]))
    print("has resized file...\n")
    min_wdw_sz = (64, 128)
    step_size = (10, 10)
    downscale = 1.25

    clf = joblib.load(os.path.join("model", 'svm.model'))
    ...
    for im_scaled in pyramid_gaussian(im, downscale = downscale):
        ...
        for (x, y, im_window) in sliding_window(im_scaled, min_wdw_sz, step_size):
            ...
            fd = hog(im_window, 9, [6,6], [2,2], visualise=False, transform_sqrt=True)
            ...
            if pred == 1:
                ...
                if clf.decision_function(fd) > 0.7:
